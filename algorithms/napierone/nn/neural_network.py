...
def main():
    ...
    with tf.device('/GPU:0'):
        ...
        train_dataset = train.filter(filter_len).map(get_segments, num_parallel_calls=tf.data.AUTOTUNE, deterministic=False).map(statistics, num_parallel_calls=tf.data.AUTOTUNE, deterministic=False).prefetch(tf.data.AUTOTUNE).cache()#.cache('./cache/'+current_time+'_svc_train_stats')

        val_dataset = val.filter(filter_len).map(get_segments, num_parallel_calls=tf.data.AUTOTUNE, deterministic=False).map(statistics, num_parallel_calls=tf.data.AUTOTUNE, deterministic=False).prefetch(tf.data.AUTOTUNE).cache()#.cache('./cache/'+current_time+'_svc_val_stats')

        test_dataset = test.filter(filter_len).map(get_segments, num_parallel_calls=tf.data.AUTOTUNE, deterministic=False).map(statistics, num_parallel_calls=tf.data.AUTOTUNE, deterministic=False).prefetch(tf.data.AUTOTUNE).cache()#.cache('./cache/'+current_time+'_svc_test_stats')print('Start DAA training')
        ...
        result_csv_file = str(NUM_SAMPLES)+'_'+str(list(range(1)))+'F_'+str(SEG_LEN)+'_daa_train_tot_results.csv'
    ...
    ##SAVE TEST RESULTS INTO CSV FILE
    #print('Saving test results')
    #result_csv_file = current_time+ '_' +str(NUM_SAMPLES)+'_'+str(list(range(1,SEG+1)))+'F_'+str(SEG_LEN)+'_nn_test_results.csv'

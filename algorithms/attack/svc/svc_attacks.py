...
def main():
    ...
    result_csv_file = str(NUM_SAMPLES)+'_'+str(list(range(1,SEG+1)))+'F_'+str(SEG_LEN)+'_daa_attacks_svc_val_results.csv'
    with open(histrory_dir + current_time + '_' + result_csv_file, mode='w') as csvfile:
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
        writer.writeheader()
    ...
    #SAVE TEST RESULTS INTO CSV FILE
    print('Saving test results')
    result_csv_file = str(NUM_SAMPLES)+'_'+str(list(range(1,SEG+1)))+'F_'+str(SEG_LEN)+'_daa_attacks_svc_test_results.csv'
    with open(histrory_dir + current_time + '_' + result_csv_file, mode='w') as csvfile:
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
        writer.writeheader()

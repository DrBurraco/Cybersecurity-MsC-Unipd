...
def main():
    ...
    with tf.device('/GPU:0'):
        ...
        linear_svc_val_results = []
        linear_svc_test_results = []
        daa_train_results = []
        daa_test_results = []
        ...
        ##for seg in range(SEG):
            ##print(daa_train_metrics[0][d_t_indexes[seg]])
            ##print(daa_train_metrics[1][d_t_indexes[seg]])
            ##print(daa_train_metrics[2][d_t_indexes[seg]])
            ##print(daa_train_metrics[3][d_t_indexes[seg]])
        ...
        result_csv_file = str(NUM_SAMPLES)+'_'+str(list(range(2,SEG+1)))+'F_'+str(SEG_LEN)+'_daa_train_tot_results.csv'
        ...
        #for seg,seg_len,stat in product(range(1,SEG+1),range(1,SUB_PARTS+1),STATISTICS):
            ...
            #with parallel_backend('threading', n_jobs=24):
                ...
                #linear_svc.fit(train_features,np.ravel(train_labels))


                #val_labels_pred = linear_svc.predict(val_features)
                ...
                #test_labels_pred = linear_svc.predict(test_features)
                ...
                #linear_svc_test_results.append({
                    #'Data Type': 'standard',
                    #'Segments': seg,
                    #'Segments Length': seg_len*SUB_LEN,
                    #'Statistics': stat,
    ...
    #result_csv_file = str(NUM_SAMPLES)+'_'+str(list(range(1,SEG+1)))+'F_'+str(SEG_LEN)+'_svc_val_results.csv'
    #with open(histrory_dir + current_time + '_' + result_csv_file, mode='w') as csvfile:
        #writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
        #writer.writeheader()
        #writer.writerows(linear_svc_val_results)

    ##SAVE TEST RESULTS INTO CSV FILE
    #print('Saving test results')
    #result_csv_file = str(NUM_SAMPLES)+'_'+str(list(range(1,SEG+1)))+'F_'+str(SEG_LEN)+'_svc_test_results.csv'
    #with open(histrory_dir + current_time + '_' + result_csv_file, mode='w') as csvfile:
        #writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
        #writer.writeheader()
        #writer.writerows(linear_svc_test_results)
